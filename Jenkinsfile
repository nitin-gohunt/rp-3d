...
def UpdateTag() {
    env.staging_tag = sh([returnStdout: true, label: 'save staging_tag', script: "cat ./manifest_staging/variables.tfvars | grep \${DOCKER_APP}_Version | tr -d \")"]).toString().trim()
    env.IMAGE_TAG = sh([returnStdout: true, label: 'save updated_image_tag', script: "echo \${staging_tag} | awk -F '-Snap' '{print \$1}' | awk -F '=' '{print \$2}'"]).toString().trim()
}
